...
class StorePaymentMethod(db.Model):
    ...
    method = db.Column(db.Enum('pix', 'credit_card', 'debit_card', 'boleto'), nullable=False)
    ...
    __table_args__ = (
        db.UniqueConstraint('store_id', 'method', name='unique_store_payment_method'),
    )
    
    def __repr__(self):
        return f'<StorePaymentMethod {self.method} for store {self.store_id}>'
